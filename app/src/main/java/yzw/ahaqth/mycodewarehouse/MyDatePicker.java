...
public class MyDatePicker extends LinearLayout {
    ...
    private void currentMonthClick() {
        if(isMultiSelect){
            selectDateRange = getMonthStartAndEndLong(monthDay);
            isFirstSelect = true;
            if(clickListener!=null)
                clickListener.onMultiClick(false);
            changeDateViewUI();
    ...
    private long[] getMonthStartAndEndLong(long date) {
        ...
        long end = start + c.getActualMaximum(Calendar.DAY_OF_MONTH) * 86400000 - 1;
        times[0] = start;
        times[1] = end;
        return times;
    }
